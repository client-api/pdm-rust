/*
 * Proxmox Datacenter Manager API
 *
 * Generated from apidoc.js. NOT an official Proxmox specification. See https://pdm.proxmox.com/docs/api-viewer/ for the upstream documentation.
 *
 * The version of the OpenAPI document: 9.x
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PveGetRealmsResponseDataInner {

    /// A comment. The GUI use this text when you select a domain (Realm) on the login window.
    #[serde(rename = "comment", skip_serializing_if = "Option::is_none")]
    pub comment: Option<String>,

    /// FIXME: Missing description in PVE.
    #[serde(rename = "realm")]
    pub realm: String,

    /// Two-factor authentication provider.
    #[serde(rename = "tfa", skip_serializing_if = "Option::is_none")]
    pub tfa: Option<models::PdmPveTfaEnum>,

    /// FIXME: Missing description in PVE.
    #[serde(rename = "type")]
    pub r#type: String,


}

impl PveGetRealmsResponseDataInner {
    pub fn new(realm: String, r#type: String) -> PveGetRealmsResponseDataInner {
        PveGetRealmsResponseDataInner {
            
            comment: None,
            
            realm,
            
            tfa: None,
            
            r#type,
            
        }
    }
}


