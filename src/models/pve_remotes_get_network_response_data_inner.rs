/*
 * Proxmox Datacenter Manager API
 *
 * Generated from apidoc.js. NOT an official Proxmox specification. See https://pdm.proxmox.com/docs/api-viewer/ for the upstream documentation.
 *
 * The version of the OpenAPI document: 9.x
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PveRemotesGetNetworkResponseDataInner {

    /// Set to true if the interface is active.
    #[serde(rename = "active", skip_serializing_if = "Option::is_none")]
    pub active: Option<bool>,

    /// IP address.
    #[serde(rename = "address", skip_serializing_if = "Option::is_none")]
    pub address: Option<String>,

    /// IP address.
    #[serde(rename = "address6", skip_serializing_if = "Option::is_none")]
    pub address6: Option<String>,

    /// Automatically start interface on boot.
    #[serde(rename = "autostart", skip_serializing_if = "Option::is_none")]
    pub autostart: Option<bool>,

    /// Specify the primary interface for active-backup bond.
    #[serde(rename = "bond-primary", skip_serializing_if = "Option::is_none")]
    pub bond_primary: Option<String>,

    /// Bonding mode.
    #[serde(rename = "bond_mode", skip_serializing_if = "Option::is_none")]
    pub bond_mode: Option<models::PdmPveRemotesBondModeEnum>,

    /// Selects the transmit hash policy to use for slave selection in balance-xor and 802.3ad modes.
    #[serde(rename = "bond_xmit_hash_policy", skip_serializing_if = "Option::is_none")]
    pub bond_xmit_hash_policy: Option<models::PdmBondXmitHashPolicyEnum>,

    /// The bridge port access VLAN.
    #[serde(rename = "bridge-access", skip_serializing_if = "Option::is_none")]
    pub bridge_access: Option<i64>,

    /// Bridge port ARP/ND suppress flag.
    #[serde(rename = "bridge-arp-nd-suppress", skip_serializing_if = "Option::is_none")]
    pub bridge_arp_nd_suppress: Option<bool>,

    /// Bridge port learning flag.
    #[serde(rename = "bridge-learning", skip_serializing_if = "Option::is_none")]
    pub bridge_learning: Option<bool>,

    /// Bridge port multicast flood flag.
    #[serde(rename = "bridge-multicast-flood", skip_serializing_if = "Option::is_none")]
    pub bridge_multicast_flood: Option<bool>,

    /// Bridge port unicast flood flag.
    #[serde(rename = "bridge-unicast-flood", skip_serializing_if = "Option::is_none")]
    pub bridge_unicast_flood: Option<bool>,

    /// Specify the interfaces you want to add to your bridge.
    #[serde(rename = "bridge_ports", skip_serializing_if = "Option::is_none")]
    pub bridge_ports: Option<String>,

    /// Specify the allowed VLANs. For example: '2 4 100-200'. Only used if the bridge is VLAN aware.
    #[serde(rename = "bridge_vids", skip_serializing_if = "Option::is_none")]
    pub bridge_vids: Option<String>,

    /// Enable bridge vlan support.
    #[serde(rename = "bridge_vlan_aware", skip_serializing_if = "Option::is_none")]
    pub bridge_vlan_aware: Option<bool>,

    /// IPv4 CIDR.
    #[serde(rename = "cidr", skip_serializing_if = "Option::is_none")]
    pub cidr: Option<String>,

    /// IPv6 CIDR.
    #[serde(rename = "cidr6", skip_serializing_if = "Option::is_none")]
    pub cidr6: Option<String>,

    /// Comments
    #[serde(rename = "comments", skip_serializing_if = "Option::is_none")]
    pub comments: Option<String>,

    /// Comments
    #[serde(rename = "comments6", skip_serializing_if = "Option::is_none")]
    pub comments6: Option<String>,

    /// Set to true if the interface physically exists.
    #[serde(rename = "exists", skip_serializing_if = "Option::is_none")]
    pub exists: Option<bool>,

    /// The network families.
    #[serde(rename = "families", skip_serializing_if = "Option::is_none")]
    pub families: Option<Vec<models::PdmPveRemotesInlineEnum>>,

    /// Default gateway address.
    #[serde(rename = "gateway", skip_serializing_if = "Option::is_none")]
    pub gateway: Option<String>,

    /// Default ipv6 gateway address.
    #[serde(rename = "gateway6", skip_serializing_if = "Option::is_none")]
    pub gateway6: Option<String>,

    /// Network interface name.
    #[serde(rename = "iface")]
    pub iface: String,

    /// The link type.
    #[serde(rename = "link-type", skip_serializing_if = "Option::is_none")]
    pub link_type: Option<String>,

    /// The network configuration method for IPv4.
    #[serde(rename = "method", skip_serializing_if = "Option::is_none")]
    pub method: Option<models::PdmPveRemotesMethodEnum>,

    /// The network configuration method for IPv4.
    #[serde(rename = "method6", skip_serializing_if = "Option::is_none")]
    pub method6: Option<models::PdmPveRemotesMethodEnum>,

    /// MTU.
    #[serde(rename = "mtu", skip_serializing_if = "Option::is_none")]
    pub mtu: Option<i32>,

    /// Network mask.
    #[serde(rename = "netmask", skip_serializing_if = "Option::is_none")]
    pub netmask: Option<String>,

    /// Network mask.
    #[serde(rename = "netmask6", skip_serializing_if = "Option::is_none")]
    pub netmask6: Option<i32>,

    /// A list of additional interface options for IPv4.
    #[serde(rename = "options", skip_serializing_if = "Option::is_none")]
    pub options: Option<Vec<String>>,

    /// A list of additional interface options for IPv6.
    #[serde(rename = "options6", skip_serializing_if = "Option::is_none")]
    pub options6: Option<Vec<String>>,

    /// Specify the interfaces used by the bonding device.
    #[serde(rename = "ovs_bonds", skip_serializing_if = "Option::is_none")]
    pub ovs_bonds: Option<String>,

    /// The OVS bridge associated with a OVS port. This is required when you create an OVS port.
    #[serde(rename = "ovs_bridge", skip_serializing_if = "Option::is_none")]
    pub ovs_bridge: Option<String>,

    /// OVS interface options.
    #[serde(rename = "ovs_options", skip_serializing_if = "Option::is_none")]
    pub ovs_options: Option<String>,

    /// Specify the interfaces you want to add to your bridge.
    #[serde(rename = "ovs_ports", skip_serializing_if = "Option::is_none")]
    pub ovs_ports: Option<String>,

    /// Specify a VLan tag (used by OVSPort, OVSIntPort, OVSBond)
    #[serde(rename = "ovs_tag", skip_serializing_if = "Option::is_none")]
    pub ovs_tag: Option<i32>,

    /// The order of the interface.
    #[serde(rename = "priority", skip_serializing_if = "Option::is_none")]
    pub priority: Option<i64>,

    /// Specify the interfaces used by the bonding device.
    #[serde(rename = "slaves", skip_serializing_if = "Option::is_none")]
    pub slaves: Option<String>,

    /// Network interface type
    #[serde(rename = "type")]
    pub r#type: models::PdmPveRemotesTypeEnum2,

    /// The uplink ID.
    #[serde(rename = "uplink-id", skip_serializing_if = "Option::is_none")]
    pub uplink_id: Option<String>,

    /// vlan-id for a custom named vlan interface (ifupdown2 only).
    #[serde(rename = "vlan-id", skip_serializing_if = "Option::is_none")]
    pub vlan_id: Option<i32>,

    /// The VLAN protocol.
    #[serde(rename = "vlan-protocol", skip_serializing_if = "Option::is_none")]
    pub vlan_protocol: Option<models::PdmVlanProtocolEnum>,

    /// Specify the raw interface for the vlan interface.
    #[serde(rename = "vlan-raw-device", skip_serializing_if = "Option::is_none")]
    pub vlan_raw_device: Option<String>,

    /// The VXLAN ID.
    #[serde(rename = "vxlan-id", skip_serializing_if = "Option::is_none")]
    pub vxlan_id: Option<i64>,

    /// The VXLAN local tunnel IP.
    #[serde(rename = "vxlan-local-tunnelip", skip_serializing_if = "Option::is_none")]
    pub vxlan_local_tunnelip: Option<String>,

    /// The physical device for the VXLAN tunnel.
    #[serde(rename = "vxlan-physdev", skip_serializing_if = "Option::is_none")]
    pub vxlan_physdev: Option<String>,

    /// The VXLAN SVC node IP.
    #[serde(rename = "vxlan-svcnodeip", skip_serializing_if = "Option::is_none")]
    pub vxlan_svcnodeip: Option<String>,


}

impl PveRemotesGetNetworkResponseDataInner {
    pub fn new(iface: String, r#type: models::PdmPveRemotesTypeEnum2) -> PveRemotesGetNetworkResponseDataInner {
        PveRemotesGetNetworkResponseDataInner {
            
            active: None,
            
            address: None,
            
            address6: None,
            
            autostart: None,
            
            bond_primary: None,
            
            bond_mode: None,
            
            bond_xmit_hash_policy: None,
            
            bridge_access: None,
            
            bridge_arp_nd_suppress: None,
            
            bridge_learning: None,
            
            bridge_multicast_flood: None,
            
            bridge_unicast_flood: None,
            
            bridge_ports: None,
            
            bridge_vids: None,
            
            bridge_vlan_aware: None,
            
            cidr: None,
            
            cidr6: None,
            
            comments: None,
            
            comments6: None,
            
            exists: None,
            
            families: None,
            
            gateway: None,
            
            gateway6: None,
            
            iface,
            
            link_type: None,
            
            method: None,
            
            method6: None,
            
            mtu: None,
            
            netmask: None,
            
            netmask6: None,
            
            options: None,
            
            options6: None,
            
            ovs_bonds: None,
            
            ovs_bridge: None,
            
            ovs_options: None,
            
            ovs_ports: None,
            
            ovs_tag: None,
            
            priority: None,
            
            slaves: None,
            
            r#type,
            
            uplink_id: None,
            
            vlan_id: None,
            
            vlan_protocol: None,
            
            vlan_raw_device: None,
            
            vxlan_id: None,
            
            vxlan_local_tunnelip: None,
            
            vxlan_physdev: None,
            
            vxlan_svcnodeip: None,
            
        }
    }
}


