/*
 * Proxmox Datacenter Manager API
 *
 * Generated from apidoc.js. NOT an official Proxmox specification. See https://pdm.proxmox.com/docs/api-viewer/ for the upstream documentation.
 *
 * The version of the OpenAPI document: 9.x
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct RemotesMetricCollectionCreateTriggerRequest {

    /// Remote ID.
    #[serde(rename = "remote", skip_serializing_if = "Option::is_none")]
    pub remote: Option<String>,


}

impl RemotesMetricCollectionCreateTriggerRequest {
    pub fn new() -> RemotesMetricCollectionCreateTriggerRequest {
        RemotesMetricCollectionCreateTriggerRequest {
            
            remote: None,
            
        }
    }
}


